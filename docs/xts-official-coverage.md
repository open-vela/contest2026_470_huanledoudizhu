...
| N/A        |    4 | 板卡未引出测试所需硬件接口，官方用例不适用  |
| NOT TESTED |    7 | 没有按官方步骤执行和留存完整结果            |

当前可以对外声明的完整结果是：系统内核 13 项、烧写、`fstest`、`ramtest`、RAM 随机读写、`reboot`、Cold boot 和 12 小时断网静置均已通过，RAM 和 Flash 占用统计均已完成。GPIO、I2C/SPI、UART 和 UART 文件传输 4 项因板卡未引出测试所需硬件接口不适用。剩余 7 项待测。Cold boot 和 Reboot 的 10 次平均启动时间尚未采集。不能把 `ostest` 内部的 watchdog、POSIX timer、scheduler lock 等子测试写成独立驱动用例通过。
...
| 1.3.6  | GPIO / `cmocka_driver_gpio`                 | N/A        | 板卡为声网 AI 开发套件 R1，GPIO 全部被板载外设占用（GPIO40/41 LED、GPIO50 音频 PA、GPIO10/11 控制台 UART），未引出可短接的空余 GPIO 脚，官方杜邦线回环测试不适用 |
| 1.3.7  | I2C/SPI / `cmocka_driver_i2c_spi`           | N/A        | 板载传感器为集成陀螺仪（非 BMI160），I2C/SPI 接口未引出，官方 BMI160 传感器测试不适用 |
| 1.3.10 | UART / `cmocka_driver_uart`                 | N/A        | 板卡仅引出 UART0 作 NSH 控制台，无第二路 UART 接口，官方独立串口收发测试不适用 |
| 1.3.11 | UART 文件传输                               | N/A        | 同 1.3.10，无第二路 UART 接口，不适用                        |
...
| 能力              | 板上硬件                                   | 当前观察                                                     | 官方品类测试结论                          | 后续重点                                                     |
| ----------------- | ------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------- | ------------------------------------------------------------ |
| 2.4 GHz Wi-Fi STA | 板载 Wi-Fi                                 | 基础联网正常：已完成关联、CCMP 握手、DHCP 获取地址、NTP 启动和数据收发 | 基础链路已验证；官方专项用例尚未逐项 PASS | 补充连接/断开、扫描、WAPI show/sense、ping、TCP/UDP `iperf` 并保存结果 |
| BLE               | 板载蓝牙（用于配网）                       | 蓝牙配网链路已接入                                           | NOT TESTED                                | 广播、扫描、配对及 Wi-Fi 共存                                |
...
| GPIO/LED          | GPIO40/41 红绿 LED、GPIO50 音频 PA         | LED 状态反馈已接入                                           | N/A（通用 1.3.6 板卡未引出空余 GPIO）     | 板载 LED 状态反馈已验证；按键/PWM 等板载能力按品类用例另行测试 |
| Audio             | 双麦克风阵列 + 本地 AEC、扬声器、GPIO50 PA | 模拟 PCM 诊断链路已接入                                      | 尚未完成标准 Audio 用例                   | audio upper-half、双麦录音、扬声器播放、AEC 回环             |
| Camera            | 板载摄像头                                 | 当前仍待标准驱动适配                                         | NOT TESTED                                | H.264/H.265 编码、摄像头预览链路                             |
| 显示屏            | 双屏协同显示                               | 当前仍待标准驱动适配                                         | NOT TESTED                                | framebuffer/LCD 驱动、双屏刷新                               |
| 触屏              | 电容触屏                                   | 当前仍待标准驱动适配                                         | NOT TESTED                                | 触摸事件输入、手势识别                                       |
| 陀螺仪            | 板载陀螺仪（运动感知）                     | 当前仍待标准驱动适配                                         | NOT TESTED                                | 倾斜/手势控制、传感器数据读取                                |

Wi-Fi 当前可正常联网，启动日志已证明关联、加密握手、DHCP、NTP 和数据收发链路工作；由于尚未严格执行并保存每个官方专项条目的预期结果，专项结论仍不记为全部 PASS。
...
- RAM 占用统计：`free`
- Flash 占用统计：`df -h` + 厂商分区占用表

**硬件不适用（4 项）：**

- GPIO：板卡未引出可短接的空余 GPIO 脚
- I2C/SPI：板载传感器非 BMI160，接口未引出
- UART：仅一路控制台 UART，无第二路接口
- UART 文件传输：同 UART，无第二路接口
